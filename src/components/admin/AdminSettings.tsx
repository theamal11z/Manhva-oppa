...
const AdminSettings: React.FC = () => {
  ...
  return (
    <div className="space-y-8">
            ...
            {adminEmails.map((email, idx) => (
              <li key={email} className="flex items-center justify-between">
                ...
                <button
                  ...
                  onClick={() => setAdminEmails(adminEmails.filter((e, i) => i !== idx))}
      ...
      <section className="p-4 border border-white/10 bg-black/20 rounded-lg">
        <h3 className="text-lg font-medium mb-2">Moderation & Community</h3>
        <div className="flex items-center mb-2">
          <input type="checkbox" id="registration" checked={registrationEnabled} onChange={e => setRegistrationEnabled(e.target.checked)} />
        ...
        <label className="block mb-2">
          ...
          <input type="text" className="manga-input" value={blockedTags} onChange={e => setBlockedTags(e.target.value)} />
        </label>
      </section>

      <button className="manga-border px-6 py-2 mt-6" onClick={handleSave}>Save Settings</button>
    </div>
  );
};
