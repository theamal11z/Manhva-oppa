...
const Reader: React.FC = () => {
  ...
  const [currentMangaStatus, setCurrentMangaStatus] = useState<string>('reading');
  ...
  // Check current manga status and update reading status when chapter loads
  useEffect(() => {
    const checkAndUpdateStatus = async () => {
      if (user && mangaId && chapterId) {
        try {
          // First check if this manga is already in the user's reading list
          ...
          // If manga has a status, use that status, otherwise mark as 'reading'
          const statusToUse = data?.status || 'reading';
          setCurrentMangaStatus(statusToUse);
          
          // Only auto-update status if not "dropped" or "on_hold"
          if (!['dropped', 'on_hold'].includes(statusToUse)) {
            await updateReadingStatus(user.id, mangaId, statusToUse, Number(chapterId));
            console.log(`Started reading chapter ${chapterId} with status: ${statusToUse}`);
          } else {
            // For dropped/on_hold, just log but don't update the status
            console.log(`Viewing chapter ${chapterId} of manga with status: ${statusToUse}`);
          }
        } catch (err) {
  ...
  useEffect(() => {
    ...
    // Also only update if manga is not dropped or on hold
    if (readingMode === 'single' && user && mangaId && chapterId && pages.length > 0 && 
        !['dropped', 'on_hold'].includes(currentMangaStatus)) {
      // If user is at the last page, consider the chapter completed
      if (currentPage === pages.length) {
        const markAsRead = async () => {
          try {
            // Keep the same status (reading or completed) but update the current chapter
            await updateReadingStatus(user.id, mangaId, currentMangaStatus, Number(chapterId));
            console.log(`Completed chapter ${chapterId} with status ${currentMangaStatus} (reached last page)`);
          } catch (err) {
        ...
        if (currentPage % 3 === 0) {
          const updateProgress = async () => {
            try {
              await updateReadingStatus(user.id, mangaId, currentMangaStatus, Number(chapterId));
              console.log(`Updated reading progress: ${currentPage}/${pages.length} with status ${currentMangaStatus}`);
            } catch (err) {
  ...
  }, [user, mangaId, chapterId, currentPage, pages.length, readingMode, currentMangaStatus]);
  ...
  const goToNextChapter = () => {
    if (hasNextChapter && nextChapterNumber !== null) {
      // Mark current chapter as read before navigating, maintaining the current status
      if (user && mangaId && chapterId) {
        // If manga is on_hold or dropped, don't update the status
        if (!['dropped', 'on_hold'].includes(currentMangaStatus)) {
          updateReadingStatus(user.id, mangaId, currentMangaStatus, Number(chapterId))
            .then(() => console.log(`Marked chapter ${chapterId} as read with status: ${currentMangaStatus}`));
        }
      }
  ...
  // Change reading status handlers
  const updateMangaStatus = async (newStatus: 'reading' | 'completed' | 'on_hold' | 'dropped' | 'plan_to_read') => {
    if (!user || !mangaId) return;
    
    try {
      setMarkingCompleted(true);
      
      // Determine the appropriate chapter number to set
      let chapterToSet = Number(chapterId);
      
      // For completed status, we want to set all chapters as read
      if (newStatus === 'completed') {
        // Get the highest chapter number for this manga
        ...
        if (chapters && chapters.length > 0) {
          chapterToSet = chapters[0].chapter_number;
        }
      }
      
      // Update the status in the database
      await updateReadingStatus(user.id, mangaId, newStatus, chapterToSet);
      
      // Update local state
      setCurrentMangaStatus(newStatus);
      setCompletedSuccess(`Manga marked as ${newStatus.replace('_', ' ')}`);
      setTimeout(() => setCompletedSuccess(null), 3000);
      
      console.log(`Updated manga status to ${newStatus} at chapter ${chapterToSet}`);
    } catch (err) {
        ...
        <div className="manga-panel p-4 bg-black/50 sm:transform sm:-rotate-2 flex justify-between items-center z-10">
          ...
          <div className="flex items-center space-x-2">
            ...
            <button
              ...
              className={`fixed bottom-20 left-4 z-40 manga-panel p-2 hover:text-red-500 transition-all transform ${showControls ? 'opacity-100' : 'opacity-0'} ${!controlAutoHide ? 'opacity-100' : ''}`}
            >
              <List className="w-6 h-6" />
              <span className="absolute -top-2 -right-2 bg-red-500 text-white text-xs rounded-full h-5 w-5 flex items-center justify-center">
                {currentMangaStatus === 'reading' ? 'R' : 
                 currentMangaStatus === 'completed' ? 'C' : 
                 currentMangaStatus === 'on_hold' ? 'H' : 
                 currentMangaStatus === 'dropped' ? 'D' : 'P'}
              </span>
            </button>
            
            {/* Status menu */}
            {showStatusMenu && (
              <div className="fixed bottom-20 left-16 z-40 manga-panel bg-black/90 p-4 transform -rotate-1 border-2 border-red-500">
                <h3 className="text-lg font-bold mb-3">Reading Status</h3>
                <div className="space-y-2">
                  <button 
                    onClick={() => updateMangaStatus('reading')} 
                    className={`w-full text-left flex items-center gap-2 p-2 ${currentMangaStatus === 'reading' ? 'bg-red-500/20 border-l-4 border-red-500' : 'hover:bg-gray-800'}`}
                  >
                    <ArrowRight className="w-5 h-5" /> Reading
                  </button>
                  <button 
                    onClick={() => updateMangaStatus('completed')} 
                    className={`w-full text-left flex items-center gap-2 p-2 ${currentMangaStatus === 'completed' ? 'bg-red-500/20 border-l-4 border-red-500' : 'hover:bg-gray-800'}`}
                  >
                    <Check className="w-5 h-5" /> Completed
                  </button>
                  <button 
                    onClick={() => updateMangaStatus('on_hold')} 
                    className={`w-full text-left flex items-center gap-2 p-2 ${currentMangaStatus === 'on_hold' ? 'bg-red-500/20 border-l-4 border-red-500' : 'hover:bg-gray-800'}`}
                  >
                    <Pause className="w-5 h-5" /> On Hold
                  </button>
                  <button 
                    onClick={() => updateMangaStatus('dropped')} 
                    className={`w-full text-left flex items-center gap-2 p-2 ${currentMangaStatus === 'dropped' ? 'bg-red-500/20 border-l-4 border-red-500' : 'hover:bg-gray-800'}`}
                  >
                    <Trash2 className="w-5 h-5" /> Dropped
                  </button>
                  <button 
                    onClick={() => updateMangaStatus('plan_to_read')} 
                    className={`w-full text-left flex items-center gap-2 p-2 ${currentMangaStatus === 'plan_to_read' ? 'bg-red-500/20 border-l-4 border-red-500' : 'hover:bg-gray-800'}`}
                  >
                    <BookMarked className="w-5 h-5" /> Plan to Read
                  </button>
                </div>
              </div>
            )}
            ...
            <button
              ...
              className={`fixed ${showSettings ? 'bottom-48' : 'bottom-4'} left-4 z-30 manga-panel p-2 hover:text-red-500 transition-all transform ${showControls ? 'opacity-100' : 'opacity-0'} ${!controlAutoHide ? 'opacity-100' : ''}`}
            >
